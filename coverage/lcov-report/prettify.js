...
"basePos": S}; W(V); T.push.apply(T, V.decorations)
...
var aj = (/\bMSIE\b/).test(navigator.userAgent); var am = /\n/g; var al = ac.sourceCode; var an = al.length; var V = 0; var aa = ac.spans; var T = aa.length; var ah = 0; var X = ac.decorations; var Y = X.length; var Z = 0; X[Y] = an; var aq, ar; for(aq = ar = 0; aq < Y;) {
 if(X[aq] !== X[aq + 2]) {
X[ar++] = X[aq++]; X[ar++] = X[aq++]
}else{
 aq += 2
...
}c(K, ["default-code"]); c(g([], [[F, /^[^<?]+/], [E, /^<!\w[^>]*(?:>|$)/], [j, /^<\!--[\s\S]*?(?:-\->|$)/], ["lang-", /^<\?([\s\S]+?)(?:\?>|$)/], ["lang-", /^<%([\s\S]+?)(?:%>|$)/], [L, /^(?:<[%?]|[%?]>)/], ["lang-", /^<xmp\b[^>]*>([\s\S]+?)<\/xmp\b[^>]*>/i], ["lang-js", /^<script\b[^>]*>([\s\S]*?)(<\/script\b[^>]*>)/i], ["lang-css", /^<style\b[^>]*>([\s\S]*?)(<\/style\b[^>]*>)/i], ["lang-in.tag", /^(<\/?[a-z][^<>]*>)/i]]), ["default-markup", "htm", "html", "mxml", "xhtml", "xml", "xsl"]); c(g([[F, /^[\s]+/, null, " \t\r\n"], [n, /^(?:\"[^\"]*\"?|\'[^\']*\'?)/, null, "\"'"]], [[m, /^^<\/?[a-z](?:[\w.:-]*\w)?|\/?>$/i], [P, /^(?!style[\s=]|on)[a-z](?:[\w:-]*\w)?/i], ["lang-uq.val", /^=\s*([^>\'\"\s]*(?:[^>\'\"\s\/]|\/(?=\s)))/], [L, /^[=<>\/]+/], ["lang-js", /^on\w+\s*=\s*\"([^\"]+)\"/i], ["lang-js", /^on\w+\s*=\s*\'([^\']+)\'/i], ["lang-js", /^on\w+\s*=\s*([^\"\'>\s]+)/i], ["lang-css", /^style\s*=\s*\"([^\"]+)\"/i], ["lang-css", /^style\s*=\s*\'([^\']+)\'/i], ["lang-css", /^style\s*=\s*([^\"\'>\s]+)/i]]), ["in.tag"]); c(g([], [[n, /^[\s\S]+/]]), ["uq.val"]); c(i({"keywords": l,
"hashComments": true,
"cStyleComments": true,
"types": e}), ["c", "cc", "cpp", "cxx", "cyc", "m"]); c(i({"keywords": "null,true,false"}), ["json"]); c(i({"keywords": R,
"hashComments": true,
"cStyleComments": true,
...
"hashComments": true,
"multiLineStrings": true}), ["bsh", "csh", "sh"]); c(i({"keywords": I,
"hashComments": true,
"multiLineStrings": true,
"tripleQuotedStrings": true}), ["cv", "py"]); c(i({"keywords": s,
"hashComments": true,
"multiLineStrings": true,
"regexLiterals": true}), ["perl", "pl", "pm"]); c(i({"keywords": f,
"hashComments": true,
"multiLineStrings": true,
...
"regexLiterals": true}), ["coffee"]); c(g([], [[C, /^[\s\S]+/]]), ["regex"]); function d(V) {
